...
def load_bitext(csv_path: str = None) -> pd.DataFrame:
    ...
    # Keep only categories defined in config
    df = df[
        df["label"].isin(config.CATEGORIES)
...
def train():
    ...
    print(
        f"Loaded {len(df)} labeled tickets "
...
class ClassifierAgent:
    ...
    def __init__(self, model_dir: str = None):
        ...
        model_dir = (
            model_dir
            or config.CLASSIFIER_MODEL_DIR
        )

        print(
            f"Loading classifier from: {model_dir}"
        )
        ...
        print(
            f"Classifier loaded on {self.device}"
        )
    ...
    def predict(
        ...
        # Remove it if the tokenizer provides it.
        inputs.pop(
            "token_type_ids",
            None
        )
